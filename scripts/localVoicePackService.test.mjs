...
async function fixture() {
  ...
  for (let index = 0; index < 114; index += 1) rows.push(`action.test_${index},action,test,测试,测试,zh-CN,default,firm`);
...
describe('LocalVoicePackService', () => {
  it('原子创建标准 Pack，复制 114 条母版且生成可发现 index', async () => {
    const { root, master, service } = await fixture();
    const result = await service.createPack({ displayName: '冷静女声', voiceId: 'voice-id-2' });
    expect(result).toMatchObject({ pack: { id: 'voice-20260822-001', name: '冷静女声', locale: 'zh-CN' }, lineCount: 114 });
    ...
    expect(detail.voiceLines).toHaveLength(114);
  });

  it('只生成安全 ASCII ID，重复创建使用安全后缀', async () => {
    const { service } = await fixture();
    expect((await service.createPack({ displayName: 'Test Voice', voiceId: 'voice-a' })).pack.id).toBe('test-voice-001');
    expect((await service.createPack({ displayName: 'Test Voice', voiceId: 'voice-b' })).pack.id).toBe('test-voice-002');
    await expect(service.createPack({ displayName: '../escape', voiceId: 'voice-c' })).rejects.toBeInstanceOf(VoicePackServiceError);
  });

  it('空字段、缺失母版和创建失败都不会留下半成品目录', async () => {
    const { root, service } = await fixture();
    await expect(service.createPack({ displayName: '', voiceId: 'voice' })).rejects.toBeInstanceOf(VoicePackServiceError);
    await expect(service.createPack({ displayName: '角色', voiceId: '' })).rejects.toBeInstanceOf(VoicePackServiceError);
    const broken = new LocalVoicePackService({ root, masterCsv: path.join(root, 'missing.csv') });
    await expect(broken.createPack({ displayName: '不会创建', voiceId: 'voice' })).rejects.toThrow();
  ...
  it('索引拒绝无效母版时回滚已原子移动的 Pack', async () => {
    ...
    await expect(service.createPack({ displayName: '索引失败', voiceId: 'voice' })).rejects.toThrow('索引刷新失败');
  ...
  it('只解析 manifest 明确声明的 MP3，拒绝路径穿越和未声明文件', async () => {
    ...
    const created = await service.createPack({ displayName: '试听', voiceId: 'voice-test' });
  ...
  it('删除用户 Pack 后立即重建索引；内置校长角色始终受服务端保护', async () => {
    const { service, root } = await fixture();
    const created = await service.createPack({ displayName: '测试删除', voiceId: 'voice-delete' });
