...
export interface GenerationResultItem { readonly key: string; readonly file: string; readonly status: string; readonly error: string | null; }
...
export interface VoiceGenerationOverride { readonly key: string; readonly line?: string; readonly ttsText?: string; }
...
function parseResult(value: unknown): GenerationResult {
  ...
  const items = value.items.filter(isRecord).flatMap((item): GenerationResultItem[] => typeof item.key === 'string' && typeof item.file === 'string' && typeof item.status === 'string'
    ? [{ key: item.key, file: item.file, status: item.status, error: typeof item.error === 'string' ? item.error : null }] : []);
