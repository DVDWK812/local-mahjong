...
function projectRoundResult(state: SeventeenStepsState): RoundResult | null {
  ...
  return {
    ...
    winners: [{
      ...
      yaku: win.score.yaku.map((item) => ({ id: item.id, name: item.name, han: item.han, yakuman: Boolean(item.yakumanValue), sourceTile: item.sourceTile })),
      yakuIds: win.score.yaku.map((item) => item.id),
      dora: win.score.dora,
      uraDora: win.score.uraDora,
      redDora: win.score.redDora,
      limitTier: win.score.limitTier,
      yakumanMultiplier: win.score.yakumanMultiplier,
      totalDora: win.score.totalDora,
      han: win.score.han,
      fu: win.score.fu,
