...
describe('ResultDialog win branch', () => {
  ...
  it('子家2番40符荣和、0本场、供托1根显示2600牌型得点和3600总计', () => {
    const state = stateWithWin(win({ pointDeltas: [-2600, 3600, 0, 0] }), { riichiSticks: 1, honba: 0 });
    const html = renderToStaticMarkup(<ResultDialog gameState={state} onReset={() => undefined} />);
    expect(html).toContain('2番40符');
    expect(html).toContain('牌型得点：2,600点');
    expect(html).toContain('供托奖励：1根 × 1000点 = 1,000点');
    expect(html).toContain('获得总计：3,600点');
    expect(html).toContain('+3,600');
  });

  it('庄家2番40符荣和显示3900牌型得点', () => {
    ...
    const html = renderToStaticMarkup(<ResultDialog gameState={state} onReset={() => undefined} />);
    expect(html).toContain('牌型得点：3,900点');
    expect(html).toContain('获得总计：3,900点');
  });

  it('1本场、供托1根、子家2番40符荣和显示3900总计', () => {
    const state = stateWithWin(win({ pointDeltas: [-2900, 3900, 0, 0] }), { riichiSticks: 1, honba: 1 });
    const html = renderToStaticMarkup(<ResultDialog gameState={state} onReset={() => undefined} />);
    expect(html).toContain('牌型得点：2,600点');
    expect(html).toContain('本场奖励：1本场 × 300点 = 300点');
    expect(html).toContain('供托奖励：1根 × 1000点 = 1,000点');
    expect(html).toContain('获得总计：3,900点');
    expect(html).toContain('+3,900');
  });

  it('自摸本场显示每名支付者额外支付100点', () => {
    const state = stateWithWin(win({
      from: null,
      winType: 'tsumo',
    ...
    const html = renderToStaticMarkup(<ResultDialog gameState={state} onReset={() => undefined} />);
    expect(html).toContain('本场奖励：1本场 × 300点 = 300点');
    expect(html).toContain('支付明细：每家额外支付100点');
    expect(html).toContain('获得总计：5,300点');
  });
  ...
  it('结果页将他家立直棒扣到立直者本人，不让放铳者代付', () => {
    ...
    const html = renderToStaticMarkup(<ResultDialog gameState={state} onReset={() => undefined} />);
    expect(html).toContain('牌型得点：8,000点');
    expect(html).toContain('供托奖励：1根 × 1000点 = 1,000点');
    expect(html).toContain('获得总计：9,000点');
  ...
  it('结果弹窗加宽且正文滚动，1366宽度下不会横向溢出', () => {
    ...
    expect(css).toContain('width: min(960px, calc(100vw - 32px))');
