...
import { LOCAL_CREATED_VOICE_SERVICE, type CreatedVoice, type VoiceDesign, validateBrowserFiles } from '../../audio/voice/CreatedVoiceService';
...
function GeneratedVoiceRegistry({ voices, state, onRefresh, onUse }: { readonly voices: readonly CreatedVoice[]; readonly state: 'loading' | 'ready' | 'error'; readonly onRefresh: () => Promise<void>; readonly onUse: (voice: CreatedVoice) => Promise<void> }) {
  ...
  return <section className="create-voice-panel create-voice-pack-screen__registry" aria-labelledby="generated-voices-title"><div className="create-voice-panel__heading"><h2 id="generated-voices-title">已生成音色</h2><button type="button" onClick={() => void onRefresh()}>刷新</button></div>{state === 'loading' ? <p role="status">正在加载本地音色…</p> : state === 'error' ? <p className="voice-management-screen__error">本地音色列表加载失败。</p> : !voices.length ? <p>尚未保存本地生成的音色。</p> : <div className="created-voice-list" role="list">{voices.map((voice) => <div className="created-voice-list__row" role="listitem" key={voice.voiceId}><button type="button" className="created-voice-list__select" onClick={() => void onUse(voice)}><strong>{voice.name}</strong><span>{voice.voiceId}</span><small>{createdVoiceSourceLabel(voice.source)}</small></button><button type="button" aria-label={`复制 ${voice.name} Voice ID`} onClick={() => void copy(voice.voiceId)}>复制 ID</button></div>)}</div>}</section>;
}
...
function VoiceClonePanel({ onStored, onUse, suggestedLanguage }: { readonly onStored: () => Promise<void>; readonly onUse: (voice: CreatedVoice) => Promise<void>; readonly suggestedLanguage: string | undefined }) {
  const [name, setName] = useState(''); const [description, setDescription] = useState(''); const [referenceText, setReferenceText] = useState(''); const [files, setFiles] = useState<File[]>([]); const [authorized, setAuthorized] = useState(false); const [busy, setBusy] = useState(false); const [error, setError] = useState<string | null>(null); const [created, setCreated] = useState<CreatedVoice | null>(null);
  const submit = async () => { if (busy) return; try { validateBrowserFiles(files); if (!authorized) throw new Error('请先确认拥有合法使用权或必要授权。'); setBusy(true); setError(null); const voice = await LOCAL_CREATED_VOICE_SERVICE.clone({ name, description, referenceText, audioFiles: files, languages: suggestedLanguage ? [suggestedLanguage] : [] }); setCreated(voice); await onStored(); } catch (cause) { setError(cause instanceof Error ? cause.message : '创建音色失败。'); } finally { setBusy(false); } };
  return <section className="create-voice-panel create-voice-pack-screen__clone" aria-labelledby="voice-clone-title"><h2 id="voice-clone-title">声音克隆</h2><label>音色名称<input value={name} onChange={(event) => setName(event.target.value)} /></label><label>参考音频<input aria-label="参考音频" type="file" accept=".wav,.mp3,.m4a,.ogg,.flac,audio/*" multiple onChange={(event) => { const next = Array.from(event.target.files ?? []); try { validateBrowserFiles(next); setFiles(next); setError(null); } catch (cause) { setError(cause instanceof Error ? cause.message : '参考音频无效。'); } }} /></label>{files.length ? <p>已选择 {files.length} 个参考音频。</p> : null}<label>参考文本（可选）<textarea value={referenceText} onChange={(event) => setReferenceText(event.target.value)} /></label><label>描述（可选）<input value={description} onChange={(event) => setDescription(event.target.value)} /></label><label className="create-voice-panel__authorization"><input type="checkbox" checked={authorized} onChange={(event) => setAuthorized(event.target.checked)} /> 我确认拥有该声音的合法使用权或已取得必要授权</label>{error ? <p className="voice-management-screen__error">{error}</p> : null}<button type="button" disabled={busy || !authorized} onClick={() => void submit()}>{busy ? '创建中…' : '创建音色'}</button>{created ? <CreatedVoiceResult voice={created} onUse={onUse} /> : null}</section>;
}
...
function VoiceDesignPanel({ onStored, onUse }: { readonly onStored: () => Promise<void>; readonly onUse: (voice: CreatedVoice) => Promise<void> }) {
  ...
  const create = async () => { if (creating) return; try { setCreating(true); setError(null); setDesign(await LOCAL_CREATED_VOICE_SERVICE.createDesign({ prompt, previewText, providers })); setCandidateId(''); } catch (cause) { setError(cause instanceof Error ? cause.message : '生成候选失败。'); } finally { setCreating(false); } };
  ...
  return <section className="create-voice-panel create-voice-pack-screen__design" aria-labelledby="voice-design-title"><h2 id="voice-design-title">音色设计</h2>{VOICE_DESIGN_PRESET_GROUPS.map((group) => <div className="create-voice-panel__preset-group" key={group.label}><h3>{group.label}</h3><div className="create-voice-panel__templates">{group.presets.map((preset) => <button type="button" className={selectedPreset === preset.id ? 'is-selected' : undefined} key={preset.id} onClick={() => setPrompt(preset.prompt)}>{preset.label}</button>)}</div></div>)}<label>音色描述<textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} /></label><label>试听文本<textarea value={previewText} onChange={(event) => setPreviewText(event.target.value)} /></label><fieldset className="create-voice-panel__providers"><legend>Provider</legend><label className="create-voice-panel__provider-option"><input type="checkbox" checked={providers.includes('fishaudio')} onChange={() => toggleProvider('fishaudio')} /> <span>Fish Audio</span></label><label className="create-voice-panel__provider-option"><input type="checkbox" checked={providers.includes('minimax')} onChange={() => toggleProvider('minimax')} /> <span>MiniMax</span></label></fieldset>{!providers.length ? <p className="voice-management-screen__error">请选择至少一个音色生成 Provider。</p> : null}<button type="button" disabled={creating || !prompt.trim() || !previewText.trim() || !providers.length} onClick={() => void create()}>{creating ? '生成候选中…' : '生成候选'}</button>{design ? <div className="voice-design-candidates"><h3>候选</h3>{design.candidates.map((candidate) => <div key={candidate.candidateId}><span>{candidate.label}（{candidate.provider}）</span><button type="button" onClick={() => void preview(candidate.candidateId)}>{playing === candidate.candidateId ? '试听中…' : '▶ 试听'}</button><button type="button" onClick={() => setCandidateId(candidate.candidateId)}>{candidateId === candidate.candidateId ? '已选择' : '选择'}</button></div>)}<label>音色名称<input value={name} onChange={(event) => setName(event.target.value)} /></label><button type="button" disabled={saving || !candidateId || !name.trim()} onClick={() => void save()}>{saving ? '保存中…' : '保存此音色'}</button></div> : null}{saved ? <CreatedVoiceResult voice={saved} onUse={onUse} /> : null}{error ? <p className="voice-management-screen__error">{error}</p> : null}</section>;
}

function CreatedVoiceResult({ voice, onUse }: { readonly voice: CreatedVoice; readonly onUse: (voice: CreatedVoice) => void }) { return <section className="create-voice-panel__result" role="status"><strong>创建成功</strong><span>名称：{voice.name}</span><span>Voice ID：{voice.voiceId}</span><button type="button" onClick={() => onUse(voice)}>使用此音色</button></section>; }
