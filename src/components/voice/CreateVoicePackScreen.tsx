...
export function GeneratedVoiceRegistry({ voices, state, onRefresh, onUse }: { readonly voices: readonly CreatedVoice[]; readonly state: 'loading' | 'ready' | 'error'; readonly onRefresh: () => Promise<void>; readonly onUse: (voice: CreatedVoice) => Promise<void> }) {
  ...
  return <section className="create-voice-panel create-voice-pack-screen__registry" aria-labelledby="generated-voices-title"><div className="create-voice-panel__heading"><h2 id="generated-voices-title">已生成音色</h2><button type="button" onClick={() => void onRefresh()}>刷新</button></div>{state === 'loading' ? <p role="status">正在加载本地音色…</p> : state === 'error' ? <p className="voice-management-screen__error">本地音色列表加载失败。</p> : !voices.length ? <p>尚未保存本地生成的音色。</p> : <div className="created-voice-list" role="list">{voices.map((voice) => { const inUse = isCreatedVoiceInUse(voice); return <div className="created-voice-list__row" role="listitem" key={voice.voiceId}><button type="button" className="created-voice-list__remove" aria-label={`删除 ${voice.name} 音色`} disabled={inUse} title={inUse ? '角色正在被使用，无法删除' : '删除音色'} onClick={() => setPendingDeletion(voice)}>−</button><button type="button" className="created-voice-list__select" onClick={() => void onUse(voice)}><strong>{voice.name}</strong><span>{voice.voiceId}</span><small>{createdVoiceSourceLabel(voice.source)}</small></button><button type="button" className="created-voice-list__copy" aria-label={`复制 ${voice.name} Voice ID`} onClick={() => void copy(voice.voiceId)}>复制 ID</button></div>; })}</div>}{pendingDeletion ? <section className="create-voice-panel__result" role="dialog"><strong>删除音色？</strong><span>本地移除只删除记录；云端删除不可恢复。</span>{removeError ? <p className="voice-management-screen__error">{removeError}</p> : null}<div><button type="button" disabled={removing} onClick={() => void remove(false)}>移除本地记录</button><button type="button" disabled={removing} onClick={() => void remove(true)}>删除 Fish 云端音色</button><button type="button" disabled={removing} onClick={() => setPendingDeletion(null)}>取消</button></div></section> : null}</section>;
}
