...
function VoiceManagementDetail({ initialDetail, voiceVolume, repository, editor, generationService, packService, onBack }: { readonly initialDetail: VoicePackDetail; readonly voiceVolume: number; readonly repository: VoicePackRepository; readonly editor: VoicePackEditingService; readonly generationService: VoiceGenerationService; readonly packService: Pick<VoicePackService, 'getPack' | 'updateVoiceLines'>; readonly onBack: () => void }) {
  ...
  const [generationPlan, setGenerationPlan] = useState<GenerationPlan | null>(null); const [generationKeys, setGenerationKeys] = useState<readonly string[] | undefined>(); const [generationOverrides, setGenerationOverrides] = useState<readonly VoiceGenerationOverride[]>([]); const [generating, setGenerating] = useState(false); const [generationProgress, setGenerationProgress] = useState<GenerationProgress | null>(null); const [generatingKey, setGeneratingKey] = useState<string | null>(null); const [generationError, setGenerationError] = useState<string | null>(null);
  ...
  const confirmGeneration = async () => {
    ...
    if (targetKeys.length === 0) { setGenerationPlan(null); return; }
...
export function GenerationPlanDialog({ plan, lines, selectedKeys, generating, progress, onCancel, onConfirm }: { readonly plan: GenerationPlan; readonly lines: readonly VoiceLine[]; readonly selectedKeys: readonly string[] | undefined; readonly generating: boolean; readonly progress: GenerationProgress | null; readonly onCancel: () => void; readonly onConfirm: () => void }) {
